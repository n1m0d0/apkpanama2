...
public class adapter_listPreReg extends BaseAdapter {

    protected Activity activity;
    protected ArrayList<obj_listPreReg> items;

    public adapter_listPreReg(Activity activity, ArrayList<obj_listPreReg> items) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        obj_listPreReg item = items.get(position);
        ...
        shapeDrawable.getPaint().setColor(Color.parseColor(item.getListColor()));
        shapeDrawable.setPadding(10, 10, 10, 10);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            ll0001.setBackground(shapeDrawable);
        } else {
            ll0001.setBackgroundColor(Color.parseColor(item.getListColor()));
        }
        ...
        values.setText(item.getKeyValue());

        TextView startDate = vi.findViewById(R.id.tvDateEventBegin);
        startDate.setText(item.getStringValue());
        ...
        imageBytes = Base64.decode(item.getListIcon(), Base64.DEFAULT);
